    ...
    def formfield_for_manytomany(self, db_field, request, **kwargs):
        if db_field.name == 'question_blocks':
            ...
            for block in all_blocks:
                if not block.marathonround_set.exists():
...
class MarathonWeekOfficialAdmin(admin.ModelAdmin):
    list_display = ('id', 'code_name', 'is_active', 'is_continuous', 'is_rating', 'rounds_display')
    ...
    def rounds_display(self, obj):
        if obj.rounds.exists():
            to_display = []
            for round in obj.rounds.order_by('date_time_start'):
                url = reverse('admin:marathon_marathonround_change', args=(f'{round.pk}', ))
                style = 'style="color: lime"' if round.is_played else ''
                text = f'({round.id}) None'
                if round.date_time_start:
                    date_time_start = timezone.localtime(round.date_time_start)
                    ...
                    text = f'({round.id}) {day}/{month}/{str(year)[:2]} в {hour}:{minute}:{second}'
    ...
    def formfield_for_manytomany(self, db_field, request, **kwargs):
        if db_field.name == 'rounds':
            ...
            for round in all_rounds:
                if not round.marathonweekofficial_set.exists() \
                        and not round.marathonweekcommunity_set.exists():
